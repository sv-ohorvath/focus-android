...
class MultitaskingTest {
    ...
    fun testVisitingMultipleSites() {
        ...
        run {
            ...
            onFloatingTabsButton()
                .check(ViewAssertions.matches(ViewMatchers.isDisplayed()))
                .check(ViewAssertions.matches(ViewMatchers.withContentDescription(Matchers.`is`("Tabs open: 2"))))
        }
        run {
            ...
            onFloatingTabsButton()
                .check(ViewAssertions.matches(ViewMatchers.isDisplayed()))
                .check(ViewAssertions.matches(ViewMatchers.withContentDescription(Matchers.`is`("Tabs open: 3"))))
        }
        run {
        ...
        run {
            ...
            onFloatingTabsButton()
                .perform(ViewActions.click())
            Espresso.onView(ViewMatchers.withText(R.string.tabs_tray_action_erase))
                .check(ViewAssertions.matches(ViewMatchers.isDisplayed()))
                .perform(ViewActions.click())
            ...
            val sessionManager =
                InstrumentationRegistry.getInstrumentation().getTargetContext().applicationContext.components.sessionManager
            TestCase.assertTrue(sessionManager.sessions.isEmpty())
        }
    }
    ...
    private fun openInNewTab() {
        Espresso.onView(ViewMatchers.withText(R.string.contextmenu_open_in_new_tab))
            .perform(ViewActions.click())
        checkNewTabPopup()
    }
